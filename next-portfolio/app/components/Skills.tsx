...
const SkillsCard = ({
  ...
  return (
    <motion.div
      variants={item}
      className="w-full sm:min-w-79.5 px-6 py-5 bg-white rounded-xl shadow-md dark:bg-neutral-800 border border-transparent hover:border-neutral-200 dark:hover:border-neutral-700 transition-all duration-300 hover:shadow-xl"
    >
      ...
      <div className="flex flex-wrap gap-2">
        {skills.map((skill, idx) => (
          <motion.span
            ...
            className={`px-3 py-1.5 text-sm font-medium rounded-full bg-linear-to-r ${proficiencyColors[skill.proficiency]} shadow-sm hover:shadow-md cursor-default transition-all duration-200`}
          >
            {skill.skill}
          </motion.span>
        ))}
